 ...
 * GUI for calculator.
 */
public class PRCalcDialog extends BaseTransformDialog implements ITransformDialog {
	/**
	 * Package name.
	 */
	...
	public PRCalcDialog(
		...
		super(parent, ivariables, baseTransformMeta, pipelineMeta, stepName);
		this.meta = (PRCalcMeta) baseTransformMeta;
	...
	public String open() {
		...
		wStepname.setText(stepName); //todo --> stepname <-- unknown string field name
		...
		wScriptFilePath = new TextVar(ivariables, gProps, SWT.SINGLE | SWT.LEFT
				| SWT.BORDER);
		...
		wInputVars = new TableView(ivariables, gProps, SWT.BORDER
		...
		wOutputVars = new TableView(ivariables, gProps, SWT.BORDER
				| SWT.FULL_SELECTION | SWT.MULTI | SWT.V_SCROLL | SWT.H_SCROLL,
		...
		Runnable fieldLoader = new Runnable() {
			public void run() {
				try {
					...
					prevFields = pipelineMeta.getPrevInfoFields(ivariables, stepName); //todo migrate is not secure
